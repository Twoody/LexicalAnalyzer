...
//void string_done();
#define string_done() {                                                 \
 if (_string_pos > (POOL_SIZE - 1)) error_fatal( ER_POOLFULL, "STRINGPOOL: string_done");  \
...
#define check_bounds(h) {if (h > POOL_SIZE - 1) error_fatal(ER_POOLBOUNDS, "STRINGPOOL: check_bounds");}
